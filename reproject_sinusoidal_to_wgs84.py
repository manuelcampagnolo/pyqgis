...
# a,b are compulsory; otherwise the projection is not well positioned
crs_sinu='+proj=sinu +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +a=6371007.181 +b=6371007.181 +no_defs'

# fn csv input file
# output layer name
# X,Y are the names of the columns with coordinates in the file
...
def create_4326_layer_from_lat_lon_txt_csv(fn,ln,X,Y,inputCRSTYPE,inputCRS,sep=","):
    #auxfn=os.path.splitext(fn)[0]+'.gpkg'
    ...
    mylayer=QgsVectorLayer(os.path.join(myfolder,lixo+'.gpkg'),ln,"ogr") # added fid column
